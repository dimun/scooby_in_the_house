...
async def _run_scraper(task_id: str, city: str, region: str, property_type: str, max_pages: int, db: Session) -> None:
    """
    ...
    try:
        ...
        async with FincaRaizScraper() as scraper:
            properties = await scraper.get_property_listings(city, region, property_type, max_pages)
            
            log_message = f"[Task {task_id}] Found {len(properties)} properties. Saving to database."
            logger.info(log_message)
            _add_log_entry(task_id, "info", log_message)
            
            # Save properties to database
            FincaRaizScraper.save_properties_to_db(db, properties)
            
        end_time = datetime.now()
        duration = (end_time - start_time).total_seconds()
        log_message = f"[Task {task_id}] Scraping completed for {property_type} in {city}, {region}. Duration: {duration}s"
        logger.info(log_message)
        _add_log_entry(task_id, "info", log_message)
        
        # Update task status with results
        task_repo.update_task_status(task_id, "completed", properties_found=len(properties))
...
async def scrape_properties(city: str, region: str, property_type: str = "casas-y-apartamentos", max_pages: int = 5, db: Session = None, task_id: str = None) -> str:
    """
...
def get_task_logs(task_id: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    """
