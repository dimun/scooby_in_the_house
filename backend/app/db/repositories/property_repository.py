...
class PropertyRepository:
    ...
    def get_property_count_by_city(self) -> List[Dict[str, Any]]:
        ...
        city_stats = self.db.query(
            Property.city, 
            self.db.func.count(Property.id).label('count')
    ...
    def get_avg_price_by_city(self) -> List[Dict[str, Any]]:
        ...
        price_stats = self.db.query(
            Property.city,
            self.db.func.avg(Property.price).label('avg_price')
        ).group_by(Property.city).all()
        
        return [{"city": city, "avg_price": avg_price} for city, avg_price in price_stats]
    ...
    def check_existing_property_urls(self, urls: List[str]) -> Dict[str, Property]:
        ...
        for existing in self.db.query(Property).filter(Property.url.in_(urls)).all():
            existing_properties[existing.url] = existing
            
        return existing_properties
    
    def update_property(self, property_obj: Property, property_data: Dict[str, Any]) -> None:
    ...
    def save_properties_batch(self, properties: List[Dict[str, Any]], base_url: Optional[str] = None) -> None:
        """
        ...
        all_urls = [prop.get('url') for prop in properties if prop.get('url')]
        
        # Get existing property URLs in a single query
        existing_properties = self.check_existing_property_urls(all_urls)
            
        logger.info(f"Found {len(existing_properties)} existing properties out of {len(properties)} scraped")
        
        # Process properties in batches
        for i, prop_data in enumerate(properties):
            try:
                # Skip properties without valid URLs or that match the base URL only
                if not prop_data.get('url') or (base_url and prop_data.get('url') == f"{base_url}/"):
                    logger.warning(f"Skipping property with invalid URL: {prop_data.get('url')}")
                    continue
                
                # Ensure all string fields are truncated to their max lengths
                if prop_data.get('title') and len(prop_data['title']) > 256:
                    prop_data['title'] = prop_data['title'][:252] + "..."
                
                if prop_data.get('property_type') and len(prop_data['property_type']) > 128:
                    prop_data['property_type'] = prop_data['property_type'][:124] + "..."
                
                if prop_data.get('city') and len(prop_data['city']) > 128:
                    prop_data['city'] = prop_data['city'][:124] + "..."
                
                if prop_data.get('region') and len(prop_data['region']) > 128:
                    prop_data['region'] = prop_data['region'][:124] + "..."
                
                if prop_data.get('surface_unit') and len(prop_data['surface_unit']) > 10:
                    prop_data['surface_unit'] = prop_data['surface_unit'][:10]
                
                # Check if property already exists in our pre-fetched list
                url = prop_data['url']
                if url in existing_properties:
                    # Update existing property
                    self.update_property(existing_properties[url], prop_data)
                    logger.debug(f"Updated existing property: {url}")
                else:
                    # Create new property
                    self.create_property(prop_data)
                    logger.debug(f"Added new property: {url}")
                
                # Commit in smaller batches to avoid large transactions
                if i > 0 and i % 50 == 0:
                    try:
                        self.db.commit()
                        logger.info(f"Committed batch of properties: {i}/{len(properties)}")
