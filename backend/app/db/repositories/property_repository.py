...
class PropertyRepository:
    ...
    def save_properties_batch(self, properties: List[PropertyCreate], base_url: Optional[str] = None) -> None:
        """
        ...
        for i, prop_data in enumerate(properties):
            try:
                ...
                if not prop_data.url or (base_url and prop_data.url == f"{base_url}/"):
                    logger.warning(f"Skipping property with invalid URL: {prop_data.url}")
                    continue
                ...
                if str(prop_data.url) in existing_properties:
                    ...
                    self.update_property(existing_properties[str(prop_data.url)], prop_data.model_dump())
