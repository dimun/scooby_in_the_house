...
async def start_scraper(
    ...
    if not request.property_types:
        raise HTTPException(
            status_code=400, detail="At least one property type is required"
        )
    ...
    return {
        "message": f"Scraping job started for {request.property_types} in {request.city}, {request.region}"
    }
...
async def get_scrape_logs(
    task_id: Optional[str] = None, limit: int = Query(10, ge=1, le=100)
):
    """
    ...
    log_messages = [
        f"[{log['timestamp']}] {log['level'].upper()}: {log['message']}" for log in logs
    ]

    if not log_messages:
        log_messages = [
            "No recent scraping logs found. Either no tasks have run or they're not being captured in memory."
        ]
